...
test_that("average embedding works", {
    ...
    ## with buckets
    ev <- vocab_embed(vocab, embs, unknown_buckets = 10, max_in_bucket = 0)
    expect_true(all(ev[, -c(1:N)] == 0))

    ev <- vocab_embed(vocab, embs, unknown_buckets = 10, max_in_bucket = 1)
    tev <- ev[, -c(1:N)]
    expect_true(all(tev - floor(tev) == 0))

    ev <- vocab_embed(vocab, embs, unknown_buckets = 10, max_in_bucket = 2)
    tev <- ev[, -c(1:N)]
    expect_equal(unique(c(tev - floor(tev))), c(0.0, 0.5))

    ev <- vocab_embed(vocab, embs, unknown_buckets = 10, max_in_bucket = 3)
    tev <- ev[, -c(1:N)]
    ...
    ev <- vocab_embed(vocab2, embs, unknown_buckets = 10, max_in_bucket = 3)
    tev <- ev[, -c(1:N)]
    expect_equal(unique(c(tev - floor(tev))), c(0, 0.5, 1/3, 2/3))
    expect_equal(ev[, 1:N], embs[, (ncol(embs) - N + 1):ncol(embs)])

    expect_equal(dim(vocab_embed(vocab2, embs, unknown_buckets = 2, max_in_bucket = 100)),
                 c(10, N + 2))
})
